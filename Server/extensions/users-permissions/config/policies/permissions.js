...
module.exports = async (ctx, next) => {
 ...
 if (ctx.state.user) {
  ...
  if (
    ...
    (ctx.request && ctx.request.query && ctx.request.query._method) ||
    (ctx.request && ctx.request.body &&ctx.request.body._method)
    ) {
    try {
      // init `id` and `isAdmin` outside of validation blocks
      let id;
      let isAdmin;
      if (ctx.request.query && ctx.request.query._method){
        ctx.request.method = ctx.request.query._method
        if(ctx.request.query.data){ctx.request.body = JSON.parse(ctx.request.query.data);}
      }
      if (ctx.request.header && ctx.request.header._method){
        ctx.request.method = ctx.request.header._method
        if(ctx.request.query.data){ctx.request.body = JSON.parse(ctx.request.query.data);}
      }
      if (ctx.request.query && ctx.request.query.token) {
        ...
        if (!token) {
          ...
          if (token.used === false){
            await strapi.query('token').update({token: ctx.request.query.token}, {used: true});
            ctx.request.query.password = token.password
            throw new Error(`Token registered: ` + token.password);
          }
          if (token.user && typeof token.token === 'string' && token.password == ctx.request.query.password) {
            id = token.user.id;
          }
          isAdmin = false;
        }
        ...
        delete ctx.request.query.password;
      ...
      } else if (ctx.request && ctx.request.header && ctx.request.header.token && ctx.request.header.password) {
        if (ctx.request.header && ctx.request.header._method){
          ctx.request.method = ctx.request.header._method
        }
        ...
        if (!token) {
          ...
          if (token.user && typeof token.token === 'string' && token.password == ctx.request.header.password) {
            id = token.user.id;
          }
          isAdmin = false;
        }
